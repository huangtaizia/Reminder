...
function wrapToTwoLines(message: string): { line1: string; line2?: string; truncated: boolean } {
  const text = message.replace(/\s+/g, ' ').trim();
  if (!text) return { line1: '', truncated: false };

  const maxCharsPerLine = 26;
  if (text.length <= maxCharsPerLine) return { line1: text, truncated: false };

  const cutAtSpaceBefore = (s: string, idx: number) => {
  ...
  if (rest.length <= maxCharsPerLine) return { line1, line2: rest, truncated: false };
...
function dataUrlForReminder(reminder: Reminder): string {
